...
2. 前端弹出选择收牌行的弹窗
3. 玩家必须选择一行来收取（吃掉该行所有牌的牛头数）
4. 30秒倒计时，超时自动选择**牛头数最少**的行
