// 文件位置: src/main/java/com/example/top_hog_server/service/GameLogicService.java
...
public class GameLogicService {
    ...
    public void playerLeavesRaw(String roomId, String webSocketSessionId, Long userId) {
        ...
        try {
            ...
            if (leavingPlayer != null) {
                ...
                // 特殊处理：如果房间内只有这一个人类玩家（其他都是机器人），且他意外断线
                // 为了避免房间一直占用资源，我们可以选择销毁房间。
                // 检查是否仅剩这一个人类玩家（包括他自己，因为他还在list中）
                long humanCount = room.getPlayers().values().stream()
                        .filter(p -> !p.isRobot())
                        .count();
                // 检查是否所有其他人类（如果有）也都是托管状态（断线）
                // 如果 humanCount == 1，说明只有他一个人类。
                if (humanCount == 1) {
                    logger.info("单人游戏（含机器人）玩家 {} 断线。计划移除房间 {}。", logPlayerName, roomId);
                    // 由于是断线，我们无法确定用户是否会马上重连（刷新页面）。
                    // 所以我们不立即销毁，而是给他一个短暂的重连窗口，或者直接销毁如果业务要求严格。
                    ...
                    scheduleRoomDestructionIfEmpty(roomId, 10000);
                }
                // 如果是在等待阶段，取消准备状态
                if (room.getGameState() == GameState.WAITING || room.getGameState() == GameState.GAME_OVER) {
    ...
    private void finalizeTurn(GameRoom room) {
        ...
        for (Player player : room.getPlayers().values()) {
            if (player.getScore() >= 66) { // 游戏结束条件：有玩家猪头数达到66
        ...
        for (PlayerCardTurnEntry turnEntry : cardsForSim) {
            ...
            if (canPlace && targetRowIdx != -1) {
                ...
                if (isMyCard) {
                    ...
                    reason = "太小! 被迫选第" + (chosenIdx + 1) + "行，拿" + bullheadsTaken + "猪头。";
                    // 模拟拿走并替换
