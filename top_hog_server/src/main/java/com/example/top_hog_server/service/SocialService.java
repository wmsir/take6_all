...
public class SocialService {
        ...
        for (Long friendId : friendIds) {
            ...
            stats.put("winRate",
                    friendHistory.isEmpty() ? 0
                            : (double) friendHistory.stream().filter(h -> h.getRank() == 1).count()
    ...
    public Map<String, Object> compareFriendStats(Long friendId) {
        ...
        // 验证是否是好友
        boolean isFriend = friendshipRepository.existsByUserIdAndFriendIdAndStatus(userId, friendId, "ACCEPTED");
        ...
        // 获取双方游戏记录
        List<GameHistory> myHistory = gameHistoryRepository.findByUserIdOrderByCreatedAtDesc(userId, null).getContent();
        List<GameHistory> friendHistory = gameHistoryRepository.findByUserIdOrderByCreatedAtDesc(friendId, null)
                .getContent();
    ...
    private Map<String, Object> calculateStats(List<GameHistory> history) {
        ...
        stats.put("avgScore",
                history.isEmpty() ? 0 : history.stream().mapToInt(GameHistory::getScore).average().orElse(0));
        stats.put("wins", history.stream().filter(h -> h.getRank() == 1).count());
        stats.put("winRate", history.isEmpty() ? 0
                : (double) history.stream().filter(h -> h.getRank() == 1).count() / history.size() * 100);
