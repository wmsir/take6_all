...
本指南将指导您如何搭建、安装和启动 Top Hog Server 项目。
...
**注意**：请确保数据库 `top_hog_db` 已经创建。如果未创建，请登录 MySQL 执行：
...
构建成功后，将在 `target` 目录下生成一个 JAR 文件，例如 `top_hog_server-0.0.1-SNAPSHOT.jar`。
...
```bash
java -jar target/top_hog_server-0.0.1-SNAPSHOT.jar
```
